...
function App() {
  ...
  // 0 - none, 1,2,3,4...n actual number of exercise
  ...
  if (appState == "run") {
    ...
    function isHandStraight(shoulder, elbow, wrist) {
      ...
      if (
        find_angle(shoulder, elbow, wrist) - 180 + ALLOWED_ELBOW_BEND_ANGLE <
        0
      ) {
        return false;
      }
    ...
    function drawLowerArmWrong(ctx, shoulder, elbow, wrist) {
      ...
      drawSegment([elbow.y, elbow.x], [wrist.y, wrist.x], "red", SCALE, ctx);
    }
      ...
      switch (currentExerciseState) {
        ...
        case "LEAN_LEFT":
          ...
          if (
            ...
            failedPositions.add(
              points.indexOf(points[index]) + ", leaning left"
            );
            drawLowerArmWrong(
              ctx,
              pose[0]["keypoints"][5],
